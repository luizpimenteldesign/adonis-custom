    ...
    if (elementos.tipo) {
        ...
        elementos.tipo.addEventListener('change', function() {
            ...
            if (this.value && this.value !== '') {
                console.log('📝 Atualizando modelos para:', this.value);
                atualizarModelos(this.value);
                
                // Verificar se deve mostrar campo de cor
...
async function carregarServicos() {
    ...
    try {
        ...
        elementos.servicosContainer.innerHTML = 
            ...
            '<strong>⚠️ Erro ao carregar serviços</strong><br>' +
...
function renderizarServicos(servicos) {
    ...
    const reparo = servicos.filter(function(s) { return s.categoria === 'Reparo'; });
    const customizacao = servicos.filter(function(s) { return s.categoria === 'Customizacao'; });
    ...
    if (reparo.length > 0) {
        ...
        reparo.forEach(function(servico) {
    ...
    if (customizacao.length > 0) {
        ...
        html += '<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px;">';
        customizacao.forEach(function(servico) {
            html += criarCheckboxServico(servico);
        });
        html += '</div>';
...
function criarCheckboxServico(servico) {
    return '<div class="checkbox-item" style="padding: 12px; background: #f9f9f9; border-radius: 8px; border-left: 3px solid #ddd; transition: all 0.2s ease; height: 100%;">' +
    ...
    files.forEach(function(file, index) {
        ...
        reader.onload = function(e) {
            ...
            preview.innerHTML = 
                ...
                '<button type="button" onclick="removerFoto(' + index + ')" style="position: absolute; top: 4px; right: 4px; background: rgba(244, 67, 54, 0.9); color: white; border: none; border-radius: 50%; width: 24px; height: 24px; cursor: pointer; font-size: 16px; line-height: 1;">×</button>';
...
async function handleFormSubmit(event) {
    ...
    const servicosIds = [];
    servicosSelecionados.forEach(function(checkbox) {
...
async function enviarFormulario(formData) {
    ...
    try {
        const response = await fetch(API_URL + '/preos.php', {
            method: 'POST',
            body: formData
        });
        ...
        // ✅ ✅ ✅ ATENÇÃO: CÓDIGO DE REDIRECIONAMENTO AQUI ✅ ✅ ✅
        // ✅ ATUALIZADO - Código de redirecionamento corrigido
if (resultado.success) {
    console.log('✅ SUCESSO DETECTADO!', resultado);
    
    // Captura correta dos dados do backend
    const token = resultado.token; // Backend retorna 'token' diretamente
    const pedidoId = resultado.preosid; // Backend retorna 'preos_id'
    
    const tipoElement = document.getElementById('tipo');
    const tipo = tipoElement ? tipoElement.value : 'Não informado';
...
console.log('✅ Form Luthieria JS v4.2 carregado');
