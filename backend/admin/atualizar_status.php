...
try {
    ...
    try {
        $stmt_hist = $conn->prepare("
            INSERT INTO status_historico (pre_os_id, status, valor_orcamento, prazo_orcamento, motivo, admin_id, criado_em)
            VALUES (:pre_os_id, :status, :valor, :prazo, :motivo, :admin_id, NOW())
        ");
        $stmt_hist->execute([
            ':pre_os_id' => $id,
            ':status'    => $status,
    ...
    } catch (PDOException $e) { /* historico opcional */ }
