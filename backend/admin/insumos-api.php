...
if ($method === 'GET') {
    try {
        if ($id) {
            ...
            if ($q) $sql .= " WHERE i.nome LIKE :q OR i.marca LIKE :q OR i.modelo LIKE :q OR i.unidade LIKE :q";
...
if ($method === 'POST') {
    ...
    try {
        $stmt = $conn->prepare("INSERT INTO insumos (nome, marca, modelo, unidade, valor_unitario, quantidade_estoque, ativo) VALUES (?,?,?,?,?,?,?)");
        $stmt->execute([$nome, $marca ?: null, $modelo ?: null, $unidade, $valor, $estoque, $ativo]);
...
if ($method === 'PUT') {
    ...
    try {
        $stmt = $conn->prepare("UPDATE insumos SET nome=?, marca=?, modelo=?, unidade=?, valor_unitario=?, quantidade_estoque=?, ativo=? WHERE id=?");
        $stmt->execute([$nome, $marca ?: null, $modelo ?: null, $unidade, $valor, $estoque, $ativo, $id]);
